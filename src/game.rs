...
impl GameComponent for Game {
    ...
    fn key_pressed(&mut self, new_direction: Key) {
        match (&self.snake.direction, new_direction) {
            (Direction::Right | Direction::Left, Key::Up | Key::W) => self.snake.direction = Direction::Up, 
            (Direction::Right | Direction::Left, Key::Down | Key::S) => self.snake.direction = Direction::Down,
            (Direction::Up | Direction::Down, Key::Left | Key::A) => self.snake.direction = Direction::Left,
            (Direction::Up | Direction::Down, Key::Right | Key::D) => self.snake.direction = Direction::Right,
            (_, _) => {},
        }
    }
